...
\subsection*{Síntesis del Modelo Mecánico}
...
Se busca obtener un modelo dinámico global no lineal del accionamiento completo, incorporando la dinámica mecánica equivalente, la dinámica electromagnética de la máquina PMSM y la dinámica térmica del estator. 
En esta primera formulación no se impone la condición de campo orientado, por lo que se considera el caso general donde la corriente del eje directo no es nula ($i_{ds}^{r}(t)\neq 0$). 
Esto implica que el torque electromagnético conservará tanto el término de alineación de imanes como el de reluctancia, resultando en un modelo fuertemente acoplado y no lineal.
...
El modelo electromagnético se desarrolla en coordenadas virtuales de rotor $qd0$. 
En este marco de referencia sincrónico, las variables eléctricas de corriente alterna se transforman en variables continuas (CC) en régimen permanente, 
lo cual simplifica el análisis dinámico y permite el diseño de reguladores de error nulo. 
Sin embargo, dado que el inversor trifásico y los sensores de medición operan físicamente sobre las variables reales de fase del estator $abc$, 
es necesario incorporar explícitamente las transformaciones matemáticas de Park.

Definiendo un vector genérico de variables trifásicas $\mathbf{f}_{abcs}(t) = \begin{bmatrix} f_{as}(t) & f_{bs}(t) & f_{cs}(t) \end{bmatrix}^{T}$, 
el cual puede representar tensiones, corrientes o flujos concatenados, se adopta la transformación de Park directa invariante en amplitud:
...
Para reconstruir las señales físicas de fase a partir de las acciones de control calculadas en el marco virtual, 
se utiliza la transformación de Park inversa:
...
La transformación requiere conocer instantáneamente la posición eléctrica del rotor $\theta_r(t)$, 
la cual se relaciona con la cinemática mecánica mediante el número de pares de polos $P_p$ de la máquina:

\begin{equation*}
    \theta_r(t) = P_p \theta_m(t), \qquad \omega_r(t) = P_p \omega_m(t).
\end{equation*}

Respecto a la componente homopolar ($0s$), dado que la PMSM posee una conexión estatórica en estrella con neutro flotante o no accesible, 
la Ley de Corrientes de Kirchhoff impone que la suma instantánea de las tres corrientes de fase sea nula. 
Al no existir un camino físico de retorno, se asume:

\begin{equation*}
	i_{0s}(t)\equiv 0.
\end{equation*}

Si adicionalmente se asume que el inversor provee una alimentación trifásica de tensión equilibrada, 
la componente homopolar de tensión también se anula ($v_{0s}(t) \equiv 0$).
...
El primer término corresponde al torque de alineación producido por el flujo de los imanes permanentes del rotor ($\lambda_m^{\prime r}$, referido al estator). 
El segundo término es el torque de reluctancia, originado por la asimetría o saliencia magnética del rotor, donde $L_d$ y $L_q$ son las inductancias propias de los ejes directo y en cuadratura respectivamente.

Agrupando los parámetros físicos constantes, se definen la constante de torque de los imanes $K_t$ y la constante de reluctancia $K_{rel}$:

\begin{equation*}
    K_t = \frac{3}{2}P_p \lambda_m^{\prime r}, \qquad K_{rel} = \frac{3}{2}P_p \left(L_d - L_q\right).
\end{equation*}

Por lo que la expresión del torque se reescribe de forma compacta como:
...
El producto cruzado de los estados $i_{ds}^{r}(t)$ e $i_{qs}^{r}(t)$ en \eqref{eq:torque_kt_krel} constituye una de las principales no linealidades estructurales del modelo dinámico.

\paragraph{Ecuaciones eléctricas en coordenadas $qd0$.}
Aplicando la Segunda Ley de Kirchhoff a los devanados del estator en el marco de referencia rotórico, 
el balance dinámico de tensiones queda definido por:
...
donde $v_{qs}^{r}$ y $v_{ds}^{r}$ son las tensiones de control aplicadas por el inversor, 
y $R_s\left(T_s^{\circ}\right)$ es la resistencia de fase del estator dependiente de la temperatura. 
Los últimos términos de cada ecuación representan las fuerzas contraelectromotrices (f.c.e.m.) dinámicamente acopladas debido a la rotación del campo magnético.

Para completar el modelo matemático de acuerdo a la implementación computacional, se incluye la ecuación de la componente homopolar, regida por la inductancia de dispersión $L_{ls}$:

\begin{equation*}
    v_{0s}(t) = R_s\left(T_s^{\circ}\right) i_{0s}(t) + L_{ls} \frac{di_{0s}(t)}{dt}.
\end{equation*}

Despejando las derivadas de corriente y sustituyendo la velocidad eléctrica por $\omega_r(t) = P_p \omega_m(t)$, 
se obtienen las ecuaciones de estado del subsistema eléctrico:
...
La ecuación dinámica de la planta mecánica equivalente referida al eje del motor, desarrollada previamente, es:
...
\begin{equation}
    \dot{\omega}_m(t) = \frac{1}{J_{eq}} \left[ K_t i_{qs}^{r} + K_{rel} i_{ds}^{r} i_{qs}^{r} - b_{eq} \omega_m - \frac{g k_l}{r} \sin\left(\frac{\theta_m}{r}\right) - \frac{T_{ld}}{r} \right].
    \label{eq:omega_global_acoplada}
\end{equation}

Esta expresión evidencia el acoplamiento electromecánico bidireccional: 
las corrientes eléctricas generan el torque que modifica la cinemática del rotor, y a su vez, 
la velocidad y posición del rotor retroalimentan las ecuaciones eléctricas \eqref{eq:dinamica_corrientes_qd0} 
a través de la f.c.e.m. y las transformaciones de Park.

\paragraph{Diagrama de bloques del subsistema electromagnético.}

La arquitectura implementada en Simulink para este subsistema se ilustra en la Fig. \ref{fig:diagrama_electromecanico_equivalente}. 
Para facilitar su interconexión física, el bloque expone como entradas las tensiones trifásicas reales $v_{as}, v_{bs}, v_{cs}$, 
la cinemática del rotor $\theta_m, \omega_m$ y la resistencia actual $R_s$.

Internamente, se aplica la transformación de Park directa para resolver las ecuaciones diferenciales \eqref{eq:dinamica_corrientes_qd0} 
y calcular el torque \eqref{eq:torque_kt_krel} en el marco virtual $qd0$. Finalmente, mediante la transformación de Park inversa, 
el bloque retorna a las coordenadas de fase para entregar como salidas las corrientes reales $i_{as}, i_{bs}, i_{cs}$, 
el torque electromagnético $T_m$ y los parámetros dinámicos $K_t, K_{rel}$.

\begin{figure}[H]
    \centering
    % Inserta la ruta y nombre exacto de tu archivo PNG aquí
    \includegraphics[width=0.95\textwidth]{img/screenshot_subsistema_electromagnetico.png}
    \caption{Diagrama de bloques del subsistema electromagnético de la máquina PMSM.}
    \label{fig:diagrama_electromecanico_equivalente}
\end{figure}

\paragraph{Resistencia de estator dependiente de la temperatura.}

Las pérdidas por efecto Joule elevan la temperatura del bobinado ($T_s^{\circ}$), lo cual a su vez incrementa la resistencia de fase del estator ($R_s$). 
Este comportamiento termodinámico se modela matemáticamente como:

\begin{equation}
    R_s\left(T_s^{\circ}\right) = R_{sREF} \left[ 1 + \alpha_{Cu} \left(T_s^{\circ} - T_{sREF}^{\circ}\right) \right],
    \label{eq:resistencia_termica}
\end{equation}

donde $R_{sREF}$ es la resistencia nominal medida a la temperatura de referencia $T_{sREF}^{\circ}$, 
y $\alpha_{Cu}$ es el coeficiente de temperatura del cobre. Esta dependencia introduce un fuerte acoplamiento térmico-eléctrico en el sistema.

\paragraph{Modelo térmico del estator.}

Las pérdidas eléctricas consideradas corresponden a la disipación resistiva. En coordenadas reales de fase, la potencia total disipada se expresa como:

\begin{equation*}
    P_{s,perd}^{abc}(t) = R_s\left(T_s^{\circ}\right) \left[ i_{as}^2(t) + i_{bs}^2(t) + i_{cs}^2(t) \right].
\end{equation*}

Aplicando las propiedades de la transformación de Park invariante en amplitud, esta misma potencia puede escribirse equivalentemente en coordenadas $qd0$ incorporando el factor $3/2$. 
Dado que $i_{0s}(t) \equiv 0$, resulta:

\begin{equation*}
    P_{s,perd}^{qd0}(t) = \frac{3}{2} R_s\left(T_s^{\circ}\right) \left[ \left(i_{qs}^{r}(t)\right)^2 + \left(i_{ds}^{r}(t)\right)^2 \right].
\end{equation*}

Considerando el balance de energía entre el calor generado y el calor disipado hacia el ambiente a través de la resistencia térmica $R_{ts-amb}$, la dinámica de la temperatura del estator (con capacidad calorífica $C_{ts}$) queda gobernada por la ecuación diferencial:

\begin{equation}
    \frac{dT_s^{\circ}}{dt} = \frac{1}{C_{ts}} \left[ \frac{3}{2} R_s\left(T_s^{\circ}\right) \left( \left(i_{qs}^{r}\right)^2 + \left(i_{ds}^{r}\right)^2 \right) - \frac{T_s^{\circ} - T_{amb}^{\circ}}{R_{ts-amb}} \right].
    \label{eq:dinamica_termica_estator}
\end{equation}

\paragraph{Diagrama de bloques del subsistema térmico.}
La Fig. \ref{fig:diagrama_termico_equivalente} muestra la implementación del modelo térmico. En concordancia con las señales disponibles físicamente a la salida del bloque electromagnético, este subsistema toma directamente como entradas las corrientes reales de fase $i_{as}, i_{bs}, i_{cs}$. Internamente, calcula la potencia disipada $P_{s,perd}^{abc}$ e integra la ecuación diferencial térmica para retroalimentar el valor actualizado de $R_s$ al resto del sistema.

\begin{figure}[H]
    \centering
    % Inserta la ruta y nombre exacto de tu archivo PNG aquí
    \includegraphics[width=0.95\textwidth]{img/screenshot_subsistema_termico.png}
    \caption{Diagrama de bloques del subsistema térmico acoplado.}
    \label{fig:diagrama_termico_equivalente}
\end{figure}

\paragraph{Variables de estado, entradas y salida.}

Para sistematizar el modelo general, se adopta el siguiente vector de estado global de quinto orden:
...
Las entradas externas del modelo se separan conceptualmente en entradas de control (manipulables por el inversor) y entradas de perturbación exógenas:

\begin{equation*}
    u_c(t) = \begin{bmatrix} v_{qs}^{r}(t) \\ v_{ds}^{r}(t) \end{bmatrix}, \qquad
    u_d(t) = \begin{bmatrix} T_{ld}(t) \\ T_{amb}^{\circ}(t) \end{bmatrix}.
\end{equation*}

Para expresar el modelo matricial en forma compacta, resulta conveniente definir el vector de entrada aumentado $u(t) \in \mathbb{R}^4$:
...
Es de vital importancia distinguir estructuralmente las salidas del sistema. La salida principal de interés para el control de movimiento es la posición angular de la carga o articulación ($q$). 
Como el modelo equivalente fue referido al eje del motor, la variable física a controlar de proceso resulta:

\begin{equation*}
    y_p(t) = q(t) = \frac{\theta_m(t)}{r} = \frac{x_1(t)}{r}.
\end{equation*}

Por lo tanto, la matriz de salida asociada a la variable controlada es $C_p = \begin{bmatrix} 1/r & 0 & 0 & 0 & 0 \end{bmatrix}$.

A su vez, el sensor de posición (encoder) se encuentra montado físicamente sobre el eje del motor, 
por lo que la salida medida disponible para la retroalimentación de estado es distinta:

\begin{equation*}
    y_s(t) = \theta_m(t) = x_1(t), \qquad C_s = \begin{bmatrix} 1 & 0 & 0 & 0 & 0 \end{bmatrix}.
\end{equation*}

Esta distinción conceptual implica que, para evaluar el error de seguimiento de una trayectoria de referencia en la articulación, se utilizará $y_p(t)$, 
pero toda lectura real proveniente de la planta se modela como $y_s(t)$ y requiere ser escalada por $1/r$ en el controlador de retroalimentación.

\paragraph{Modelo global no lineal en espacio de estados.}

Agrupando las ecuaciones diferenciales mecánicas \eqref{eq:omega_global_acoplada}, eléctricas \eqref{eq:dinamica_corrientes_qd0} y térmicas \eqref{eq:dinamica_termica_estator}, 
y sustituyendo los vectores definidos, el modelo global no lineal acoplado se resume en el siguiente sistema de ecuaciones de estado:

\begin{equation}
    \boxed{
        \begin{aligned}
            \dot{x}_1 &= x_2,\\[1mm]
            \dot{x}_2 &= \frac{1}{J_{eq}} \left[ K_t x_3 + K_{rel}x_4 x_3 - b_{eq}x_2 - \frac{g k_l}{r}\sin\left(\frac{x_1}{r}\right) - \frac{u_3}{r} \right],\\[1mm]
            \dot{x}_3 &= \frac{1}{L_q} \left[ u_1 - R_s(x_5)x_3 - \left(\lambda_m^{\prime r}+L_d x_4\right) P_p x_2 \right],\\[1mm]
            \dot{x}_4 &= \frac{1}{L_d} \left[ u_2 - R_s(x_5)x_4 + L_q x_3 P_p x_2 \right],\\[1mm]
            \dot{x}_5 &= \frac{1}{C_{ts}} \left[ \frac{3}{2}R_s(x_5)\left(x_3^2+x_4^2\right) - \frac{x_5-u_4}{R_{ts-amb}} \right],\\[2mm]
            y_p &= \frac{x_1}{r}.
        \end{aligned}
    }
...
El modelo \eqref{eq:modelo_global_nl} es fuertemente no lineal debido a múltiples factores estructurales: 
el acoplamiento cruzado de corrientes que genera el torque de reluctancia ($x_4 x_3$), 
las fuerzas contraelectromotrices inducidas que mezclan velocidades y corrientes ($x_4 x_2$ y $x_3 x_2$), 
la dependencia paramétrica de la resistencia con el estado térmico ($R_s(x_5)$), 
los términos de inyección de potencia calorífica cuadráticos ($x_3^2, x_4^2$), 
y el torque de carga gravitacional no lineal ($\sin(x_1/r)$).
...
En este proyecto se adopta la estrategia de Control Vectorial con Campo Orientado (FOC, por sus siglas en inglés), la cual impone la siguiente restricción fundamental en el eje directo:

\begin{equation}
	i_{ds}^{r}(t)\equiv 0.
	\label{eq:campo_orientado}
\end{equation}

Bajo esta condición, el flujo magnético del estator se alinea perfectamente de forma ortogonal al flujo de los imanes del rotor. 
Como consecuencia, la corriente del eje en cuadratura ($i_{qs}^r$) queda asociada de manera exclusiva e independiente a la producción de torque, 
mientras que la anulación de $i_{ds}^r$ minimiza pérdidas por efecto Joule y elimina acoplamientos estructurales indeseados.

% ---------------------------------------------------------
\paragraph{I) Ecuaciones vectoriales/matriciales LTI de estado.}

Al imponer estrictamente $i_{ds}^r = 0$, el término de reluctancia en la ecuación \eqref{eq:torque_kt_krel} se anula, 
reduciendo el torque electromagnético a una relación puramente lineal equivalente a la de una máquina de corriente continua:
...
En el dominio mecánico, la única no linealidad remanente es el torque gravitacional. Para obtener un modelo lineal alrededor de una posición de equilibrio operativa $\theta_m^o$, 
se linealiza este término por aproximación de primer orden:

\begin{equation*}
    \frac{g k_l}{r}\sin\left(\frac{\theta_m}{r}\right) \approx \frac{g k_l}{r}\sin\left(\frac{\theta_m^o}{r}\right) + K_g^o\left(\theta_m - \theta_m^o\right),
\end{equation*}

donde la rigidez gravitacional equivalente local se define como:

\begin{equation}
    K_g^o = \frac{g k_l}{r^2}\cos\left(\frac{\theta_m^o}{r}\right).
    \label{eq:kg_operacion}
\end{equation}

Para el caso particular del equilibrio estable pendular vertical ($\theta_m^o = 0$), la rigidez resulta máxima con valor $K_g^o = \frac{g k_l}{r^2}$.

En el dominio eléctrico, el controlador calcula e inyecta tensiones que cancelan las fuerzas contraelectromotrices y las variaciones de la resistencia. Esto permite definir matemáticamente entradas virtuales o auxiliares de control, denotadas como $u_q(t)$ y $u_d(t)$, que actúan sobre un subsistema eléctrico puramente de primer orden. 

Para conservar la linealidad global del espacio de estados, el acoplamiento térmico cuadrático se extrae de la matriz dinámica. La potencia disipada por efecto Joule se define como una entrada exógena auxiliar $P_J$, evitando el término no lineal proporcional a $(i_{qs}^r)^2$.

Agrupando estas consideraciones y asumiendo $R_s$ constante en el valor del punto de operación, el vector de estados del modelo Lineal Invariante en el Tiempo (LTI) equivalente resulta:

\begin{equation*}
    x_{LTI}(t) = \begin{bmatrix}
        \theta_m(t) \\ \omega_m(t) \\ i_{qs}^r(t) \\ i_{ds}^r(t) \\ T_s^{\circ}(t)
    \end{bmatrix}.
\end{equation*}

Y el sistema de ecuaciones diferenciales LTI completamente desacoplado se expresa como:

\begin{equation}
    \begin{aligned}
        \dot{\theta}_m &= \omega_m,\\[1mm]
        \dot{\omega}_m &= -\frac{K_g^o}{J_{eq}}\theta_m -\frac{b_{eq}}{J_{eq}}\omega_m +\frac{K_t}{J_{eq}}i_{qs}^r -\frac{1}{J_{eq}r}T_{ld},\\[1mm]
        \dot{i}_{qs}^r &= -\frac{R_s}{L_q}i_{qs}^r +\frac{1}{L_q}u_q,\\[1mm]
        \dot{i}_{ds}^r &= -\frac{R_s}{L_d}i_{ds}^r +\frac{1}{L_d}u_d,\\[1mm]
        \dot{T}_s^{\circ} &= -\frac{1}{R_{ts-amb}C_{ts}}T_s^{\circ} +\frac{1}{C_{ts}}P_J +\frac{1}{R_{ts-amb}C_{ts}}T_{amb}^{\circ}.
    \end{aligned}
    \label{eq:lti_ecuaciones}
\end{equation}

En operación ideal con campo orientado se asume que el lazo de corriente directo logra imponer la referencia nula, por lo que $u_d \approx 0$ en régimen estacionario.

Representando la Ecuación \eqref{eq:lti_ecuaciones} en su forma canónica matricial $\dot{x}_{LTI} = A_{LTI} x_{LTI} + B_{LTI} u_{LTI}$ e $y = C_{LTI} x_{LTI}$, se definen la matriz dinámica y de entrada correspondientes:
...
\begin{equation}
    ...
    \end{bmatrix},
    \label{eq:matrices_ab_lti}
\end{equation}

asociadas al vector de entrada ampliado $u_{LTI} = \begin{bmatrix} u_q & u_d & T_{ld} & P_J & T_{amb}^{\circ} \end{bmatrix}^{T}$. La ecuación de salida controlada mantiene su estructura cinemática:

\begin{equation}
    C_{LTI} = \begin{bmatrix} \dfrac{1}{r} & 0 & 0 & 0 & 0 \end{bmatrix}.
    \label{eq:matriz_c_lti}
\end{equation}
...
\begin{equation*}
    u_q \longrightarrow i_{qs}^r \longrightarrow T_m \longrightarrow \omega_m \longrightarrow \theta_m \longrightarrow q,
\end{equation*}

incluyendo la entrada de perturbación $T_{ld}$, la realimentación gravitacional linealizada mediante $K_g^o$, la dinámica residual de $i_{ds}^r$ y la dinámica térmica lineal de $T_s^{\circ}$.

\textit{Epígrafe sugerido para figura pendiente: Diagrama de bloques de estado del modelo LTI equivalente aumentado, con entrada auxiliar $u_q(t)$, perturbación $T_{ld}(t)$, estado eléctrico $i_{qs}^r(t)$, dinámica mecánica $\omega_m(t), \theta_m(t)$, dinámica residual de $i_{ds}^r(t)$ y dinámica térmica lineal del estator.}

\paragraph{III) Ley de control mínima para imponer $i_{ds}^{r}(t)\equiv 0$.}

Para imponer $i_{ds}^r(t)\equiv 0$, se parte de la ecuación dinámica del eje directo:

\begin{equation}
    \dot{i}_{ds}^r = \frac{v_{ds}^r - R_s i_{ds}^r + L_q i_{qs}^r P_p \omega_m}{L_d}.
    \label{eq:id_para_ley_minima}
\end{equation}

La condición deseada de operación con campo orientado es:

\begin{equation*}
    i_{ds}^r(t) \equiv 0, \qquad \dot{i}_{ds}^r(t) \equiv 0.
\end{equation*}

Reemplazando estas condiciones nulas en \eqref{eq:id_para_ley_minima}, se obtiene:

\begin{equation*}
    0 = \frac{v_{ds}^r + L_q i_{qs}^r P_p \omega_m}{L_d}.
\end{equation*}

Por lo tanto, despejando la tensión, la ley mínima de control que debe aplicarse sobre la tensión virtual del eje directo es:

\begin{equation}
    \boxed{
        v_{ds}^{r*}(t) = -L_q i_{qs}^{r}(t) P_p \omega_m(t) = -L_q i_{qs}^{r}(t) \omega_r(t).
    }
    \label{eq:ley_minima_vd}
\end{equation}

Esta ley cancela el acoplamiento natural cruzado que aparece en la ecuación del eje $d$. Para que se cumpla exactamente $i_{ds}^r(t)\equiv 0$ desde el inicio, debe asumirse la siguiente hipótesis para el estado inicial:
...
Si esta condición inicial no se cumple, la ley \eqref{eq:ley_minima_vd} no hace que $i_{ds}^r$ sea instantáneamente nula, pero sí genera una dinámica residual estable que hace decaer $i_{ds}^r$ hacia cero.

\paragraph{IV) Implementación de la ley de desacoplamiento en el modelo global no lineal.}
La ley \eqref{eq:ley_minima_vd} se implementa como un controlador parcial entre las variables medidas y el inversor. El controlador parcial no reemplaza a la planta: solamente calcula las tensiones virtuales de referencia que luego se transforman a tensiones reales de fase.

El procedimiento de implementación lógico y secuencial es el siguiente:
...
\begin{enumerate}[leftmargin=*, label=\arabic*.]
    ...
    \item Mediante la Transformación de Park directa se obtienen las corrientes $i_{qs}^r(t)$ e $i_{ds}^r(t)$.
    \item Se mide o estima la velocidad mecánica $\omega_m(t)$, y se calcula la eléctrica $\omega_r(t) = P_p\omega_m(t)$.
    \item Se calcula la tensión de desacoplamiento del eje directo mediante la ley mínima:
    \begin{equation*}
        v_{ds}^{r*}(t) = -L_q i_{qs}^r(t) P_p \omega_m(t).
    \end{equation*}
    \item Se calcula la tensión de eje $q$, sea como señal física $v_{qs}^{r*}(t)$ o como entrada auxiliar desacoplada $u_q(t)$, según el esquema de control adoptado.
    \item Se forma el vector virtual de referencias $\mathbf{v}_{qd0}^{r*}(t) = \begin{bmatrix} v_{qs}^{r*}(t) & v_{ds}^{r*}(t) & 0 \end{bmatrix}^T$.
...
\textit{Epígrafe sugerido para figura pendiente: Implementación del controlador parcial de desacoplamiento sobre el modelo no lineal completo, mostrando medición de corrientes $abc$, transformación de Park directa, cálculo de $v_{ds}^{r*}$ y $v_{qs}^{r*}$, transformación de Park inversa e inversor trifásico idealizado.}
...
la inyección de la ley mínima \eqref{eq:ley_minima_vd} conduce a una dinámica residual. Sustituyendo $v_{ds}^{r*} = -L_q i_{qs}^r P_p \omega_m$ en la ecuación diferencial de la planta \eqref{eq:id_para_ley_minima}, se obtiene:

\begin{equation*}
    \dot{i}_{ds}^r = \frac{-L_q i_{qs}^r P_p \omega_m - R_s i_{ds}^r + L_q i_{qs}^r P_p \omega_m}{L_d}.
\end{equation*}

Los términos de acoplamiento se cancelan exactamente y queda expuesta la dinámica libre:

\begin{equation}
    \boxed{ \dot{i}_{ds}^r = -\frac{R_s}{L_d}i_{ds}^r. }
    \label{eq:dinamica_residual_id}
\end{equation}

La solución analítica de \eqref{eq:dinamica_residual_id} es un decaimiento exponencial:

\begin{equation}
    i_{ds}^r(t) = i_{ds}^r(t_0) e^{-\frac{R_s}{L_d}(t-t_0)}.
    \label{eq:solucion_id_residual}
\end{equation}
...
Por lo tanto, luego de algunos múltiplos de $\tau_d$ (escasos milisegundos), el valor de $i_{ds}^r$ resulta prácticamente nulo y despreciable frente a la constante de tiempo mecánica (que es órdenes de magnitud más lenta).
...
La ley del eje $d$ cancela el acoplamiento que afecta a $i_{ds}^r$, pero la ecuación del eje $q$ todavía contiene la fuerza contraelectromotriz y el acoplamiento cruzado asociado a $i_{ds}^r$:

\begin{equation}
    \dot{i}_{qs}^r = \frac{v_{qs}^r - R_s i_{qs}^r - \left(\lambda_m^{\prime r} + L_d i_{ds}^r\right) P_p \omega_m}{L_q}.
    \label{eq:iq_con_acople}
\end{equation}

Para cancelar dicho término perturbador, se define una entrada auxiliar de control $u_q(t)$ y se aplica la siguiente ley complementaria:

\begin{equation}
    \boxed{
        v_{qs}^{r*}(t) = u_q(t) + \left(\lambda_m^{\prime r} + L_d i_{ds}^r(t)\right) P_p \omega_m(t).
    }
    \label{eq:ley_complementaria_vq}
\end{equation}

Sustituyendo la inyección de control \eqref{eq:ley_complementaria_vq} en la dinámica original \eqref{eq:iq_con_acople}, se obtiene la planta linealizada deseada:

\begin{equation}
    \boxed{
        \dot{i}_{qs}^r = -\frac{R_s}{L_q}i_{qs}^r + \frac{1}{L_q}u_q.
    }
    \label{eq:iq_desacoplado}
\end{equation}

De manera análoga, definiendo una entrada auxiliar $u_d(t)$, puede escribirse la ley general del eje directo como:

\begin{equation}
    v_{ds}^{r*}(t) = u_d(t) - L_q i_{qs}^r(t) P_p \omega_m(t),
    \label{eq:ley_general_vd}
\end{equation}

lo que, al reemplazar en la planta, conduce a:

\begin{equation}
    \dot{i}_{ds}^r = -\frac{R_s}{L_d}i_{ds}^r + \frac{1}{L_d}u_d.
    \label{eq:id_desacoplado_general}
\end{equation}

Para la estrategia de campo orientado ideal se toma $u_d(t) = 0$, de forma que $i_{ds}^r$ decae libremente a cero si hubo un error inicial, o permanece nula si $i_{ds}^r(t_0) = 0$.

Con las leyes \eqref{eq:ley_complementaria_vq} y \eqref{eq:ley_general_vd}, los canales eléctricos de eje $q$ y eje $d$ quedan estructuralmente desacoplados. El modelo resultante conserva la dinámica residual inofensiva de $i_{ds}^r$, pero permite trabajar con un modelo LTI equivalente aumentado para el diseño posterior y la sintonía clásica del control.
% ---------------------------------------------------------
...
A partir de \eqref{eq:lti_ecuaciones}, la dinámica del eje \(q\) desacoplado es
