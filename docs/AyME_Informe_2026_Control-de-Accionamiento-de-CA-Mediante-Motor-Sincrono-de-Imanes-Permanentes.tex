...
El análisis espectral de los autovalores revela que el sistema global es \textbf{marginalmente estable} a lazo abierto. Esta condición es impuesta de manera exclusiva por la inercia integradora de la posición ($p_1 = 0$). Cualquier perturbación de carga $T_l(t)$ constante y sostenida en el tiempo provocará un desplazamiento angular ilimitado de tipo rampa, indicando que el sistema no posee estabilidad BIBO (Bounded-Input Bounded-Output) para la variable de salida $\theta_m(t)$. 

No obstante, debido a que el resto de las raíces características ($p_2, p_3, p_d, p_T$) se ubican estrictamente en el semiplano izquierdo del plano $s$, la velocidad angular $\omega_m(t)$ y todos los estados internos (corrientes y temperatura) sí poseen \textbf{estabilidad asintótica parcial}. Este comportamiento es inherente a los servoaccionamientos electromecánicos y justifica analíticamente la necesidad imperativa de diseñar y cerrar un lazo de control de posición externo en etapas posteriores.

\paragraph{Conclusion ANTERIOR - MODIFICAR o JUNTAR CON LA NUEVA}
...
Las aplicaciones mecatrónicas reales están sometidas a incertidumbres operativas. En este accionamiento, la inserción de una carga útil en el extremo del brazo manipulador altera drásticamente los parámetros mecánicos equivalentes reflejados en el eje motriz. Para evaluar la robustez pasiva de la planta y comprender cómo migra la dinámica del sistema frente a estas variaciones, se realiza un análisis paramétrico por barrido (sweep) de la masa de la carga útil ($m_l$).

Como se definió en el modelado mecánico, un incremento de $m_l \in [0,\;1.5]\,\si{\kilogram}$ genera un aumento simultáneo tanto en la inercia rotacional total equivalente ($J_{eq}$) como en la fricción viscosa de la articulación reflejada ($b_{eq}$). Al proyectar estas variaciones sobre las Ecuaciones~\eqref{eq:wn_em_p3} y~\eqref{eq:zeta_em_p3}, se observa que los polos electromecánicos $p_{2,3}$ son los únicos afectados directamente por la carga mecánica.

La Figura~\ref{fig:mapa_polos_migracion} ilustra las trayectorias que trazan los polos dominantes en el plano complejo $s$ al transitar desde la condición de operación nominal o en vacío ($m_l = 0\,\si{\kilogram}$, en azul) hasta la condición de carga máxima ($m_l = 1.5\,\si{\kilogram}$, en amarillo/rojo).

\begin{figure}[H]
	\centering
	\includegraphics[width=0.95\linewidth]{img/mapa_polos_migracion_LTI_Incertidumbre_Carga.pdf}
	\caption{Migración de los polos electromecánicos conjugados $p_{2,3}$ en el plano $s$ ante el aumento progresivo de la masa de carga útil $m_l$ (desde $0\%$ a $100\%$ de carga máxima).}
	\label{fig:mapa_polos_migracion}
\end{figure}

Del análisis del mapa de polos y de la resolución analítica, se desprenden las siguientes conclusiones fundamentales respecto a la migración de propiedades dinámicas:

\begin{enumerate}
    \item \textbf{Reducción de la rapidez de respuesta:} A medida que la inercia $J_{eq}$ se incrementa por efecto de la masa útil, la frecuencia natural $\omega_{n,em}$ del sistema disminuye (trasladando los polos hacia el origen). Físicamente, esto obedece a la segunda ley de Newton para la rotación: una inercia mayor requiere una cantidad superior de torque para alcanzar la misma aceleración, volviendo al accionamiento inherentemente más \"perezoso\" para responder ante transitorios de tensión o perturbaciones.
    
    \item \textbf{Aumento del amortiguamiento relativo:} Paradójicamente, a pesar de volverse más lento, la trayectoria curva de los polos indica que el sistema se vuelve \textit{menos oscilatorio} frente a cargas pesadas. Al evaluar las fórmulas para la carga máxima ($m_l = 1.5\,\si{\kilogram}$), el factor de amortiguamiento sube considerablemente hasta alcanzar $\zeta \approx 0.77$. Si bien el aumento de fricción ($b_{eq}$) colabora disipando energía de la oscilación, el principal responsable de este efecto estabilizador es el hecho de que el término de inercia ($J_{eq}$) crece en el denominador más lentamente de lo que crece la suma de los términos disipativos.
    
    \item \textbf{Conservación de la estabilidad marginal:} A lo largo de todo el barrido paramétrico de la carga, la parte real de los polos electromecánicos ($\sigma_{em}$) permanece estrictamente confinada en el semiplano izquierdo. Asimismo, la incertidumbre mecánica no altera en absoluto la posición del polo integrador en el origen ($p_1=0$), el cero de perturbación ($z_d$) ni las raíces inobservables de alta y bajísima frecuencia ($p_d$ y $p_T$). 
\end{enumerate}

En síntesis, la inserción de carga útil no desestabiliza la planta, pero suprime su velocidad de respuesta natural. Este hallazgo será un factor crítico a considerar en la fase de síntesis del controlador en lazo cerrado, el cual deberá dotar al sistema del ancho de banda necesario para compensar la lentitud mecánica sin sacrificar los márgenes de estabilidad frente a la máxima inercia posible.
