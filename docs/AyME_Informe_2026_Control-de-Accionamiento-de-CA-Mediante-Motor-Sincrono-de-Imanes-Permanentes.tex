...
Partiendo de la ecuación del eje $d$:
...
La ley mínima del eje $d$ cancela el acoplamiento que afecta a $i_{ds}^r$, pero si la condición inicial no cumple $i_{ds}^r(t_0)=0$, dicha corriente no se anula instantáneamente sino que decae según su dinámica natural. Durante ese transitorio, la ecuación del eje $q$ conserva un término cruzado dependiente de $i_{ds}^r(t)$:
...
El término que introduce la no linealidad residual es:

\begin{equation*}
	L_d i_{ds}^{r}(t)P_p\omega_m(t),
\end{equation*}

porque contiene el producto entre dos variables dinámicas. En cambio, el término $\lambda_m^{\prime r}P_p\omega_m(t)$ corresponde a la fuerza contraelectromotriz de los imanes permanentes y es lineal respecto de $\omega_m(t)$, por lo que no es necesario cancelarlo para obtener un modelo LTI equivalente.

Para no introducir una nueva entrada auxiliar, se conserva $v_{qs}^{r}(t)$ como la tensión de eje $q$ del modelo equivalente, y se distingue la tensión efectivamente aplicada al modelo no lineal mediante el superíndice $*$:

\begin{equation}
	v_{qs}^{r*}(t)=v_{qs}^{r}(t)+v_{q,\mathrm{comp}}^{r}(t).
\end{equation}

La ley de control complementaria mínima se elige como:
...
Sustituyendo esta ley en \eqref{eq:iq_con_acople}, el término cruzado se cancela exactamente:
...
Por lo tanto, la dinámica del eje $q$ queda:
...
Esta ecuación es lineal e independiente del valor inicial de $i_{ds}^{r}(t)$. Por lo tanto, la ley complementaria mínima elimina el acoplamiento residual no lineal del eje $q$ aun cuando $i_{ds}^{r}(t_0)\neq 0$.

Combinando esta acción con la ley mínima del eje $d$,

\begin{equation*}
	v_{ds}^{r*}(t)=-L_q i_{qs}^{r}(t)P_p\omega_m(t),
\end{equation*}

se obtiene la dinámica natural desacoplada:
...
De esta forma, $i_{ds}^{r}(t)$ puede tener una condición inicial distinta de cero, pero su evolución no vuelve a introducir el acoplamiento no lineal en el eje $q$. La fuerza contraelectromotriz $\lambda_m^{\prime r}P_p\omega_m(t)$ permanece en el modelo porque es un acoplamiento lineal propio de la máquina. Si más adelante se desea diseñar un lazo interno de corriente de primer orden puro, puede agregarse una compensación adicional de fuerza contraelectromotriz, pero dicha compensación ya no forma parte de la ley mínima pedida en este inciso.
...
A partir del modelo LTI equivalente obtenido anteriormente en los incisos I) y II), se incorpora ahora la dinámica residual de la corriente en el eje directo $i_{ds}^{r}(t)$ y la dinámica térmica del estator $T_s^{\circ}(t)$. De esta manera, se obtiene un modelo LTI equivalente aumentado, en el cual se conserva como núcleo principal el modelo electromecánico de tercer orden formado por $\theta_m(t)$, $\omega_m(t)$ e $i_{qs}^{r}(t)$. De esta forma se obtiene un modelo LTI equivalente aumentado, donde el vector de estado queda definido como:
...
El modelo aumentado queda dado por el siguiente sistema de ecuaciones de estado:
...
donde la potencia eléctrica disipada por efecto Joule en el estator se calcula como:
...
En este modelo se utiliza $R_{s,\mathrm{REF}}$ constante, por lo que se desprecia el acoplamiento no lineal entre la temperatura del estator y la resistencia eléctrica de los bobinados. De esta manera, la temperatura $T_s^{\circ}(t)$ se incorpora como un estado adicional para representar la dinámica térmica del sistema, pero no realimenta a las ecuaciones eléctricas mediante una resistencia variable $R_s(T_s^{\circ})$.

La corriente $i_{ds}^{r}(t)$ también se incorpora como estado adicional porque, aunque la ley de control mínima en el eje $d$ impone su decaimiento, dicha corriente no desaparece instantáneamente. Su dinámica residual queda representada por un sistema de primer orden estable:
...
Como $R_{s,\mathrm{REF}}>0$ y $L_d>0$, se cumple que $p_d<0$, por lo que $i_{ds}^{r}(t)$ tiende exponencialmente a cero luego de un transitorio.

Finalmente, la implementación del modelo LTI equivalente aumentado en Simulink se muestra en la Figura~\ref{fig:modelo_lti_aumentado}.
...
Para comparar ambos modelos se parte de las ecuaciones electromagnéticas donde aparece explícitamente la corriente de eje directo. El torque de la PMSM puede escribirse como:
...
\begin{equation}
	...
	\right]
	i_{qs}^{r}(t).
	\label{eq:torque_kt_id}
\end{equation}

Si se evalúa alrededor de un punto de operación con \(i_{ds}^{r}(t)=I_d^o\), se define:
...
\begin{equation}
	T_m(t)=K_T(I_d^o)i_{qs}^{r}(t).
\end{equation}

Por otro lado, la ecuación de tensión del eje \(q\) es:
...
Evaluando nuevamente en \(i_{ds}^{r}(t)=I_d^o\), se define:
...
\begin{equation}
	\boxed{
		...
		P_p
		\left(
		\lambda_m^{\prime r}+L_dI_d^o
		\right)
	}
	\label{eq:ke_variable_p2}
\end{equation}

y la ecuación del eje \(q\) queda:
...
De esta forma, \(K_T(I_d^o)\) modifica la producción de torque, mientras que \(K_E(I_d^o)\) modifica la fuerza contraelectromotriz asociada a la velocidad.

\paragraph{Caso \(I_d^o=0\).}

Para campo orientado ideal:
...
La ecuación mecánica equivalente queda:
...
Este es el caso que coincide con el modelo LTI equivalente aumentado utilizado como modelo nominal de diseño.

\paragraph{Caso \(I_d^o<0\): debilitamiento de campo.}

Si se impone una corriente de eje directo negativa:

\begin{equation}
	I_d^o<0,
\end{equation}

entonces disminuye la constante de fuerza contraelectromotriz:

\begin{equation}
	K_E(I_d^o)<K_E(0).
\end{equation}

Como en la máquina considerada \(L_d>L_q\), también disminuye la constante de torque:
...
El debilitamiento de campo reduce la tensión inducida en el eje \(q\), permitiendo mayor velocidad antes de alcanzar el límite de tensión del inversor, pero reduce el torque disponible por ampere de corriente \(i_q\).

\paragraph{Caso \(I_d^o>0\): reforzamiento de campo.}

Si se impone una corriente de eje directo positiva:
...
El reforzamiento de campo aumenta el torque por ampere de eje \(q\), pero también aumenta la fuerza contraelectromotriz y la tensión requerida en el eje \(q\).

En ambos casos con \(I_d^o\neq0\), aumentan las pérdidas Joule respecto del caso \(I_d^o=0\), ya que:
