...
La Figura~\ref{fig:modelo_global_nl} muestra la implementación en Simulink del modelo global no lineal del sistema completo. En el esquema se observan los principales subsistemas que componen el accionamiento: el generador trifásico, la transformación de Park, el subsistema eléctrico de la PMSM, el subsistema mecánico equivalente, la transformación inversa de Park y el subsistema térmico del estator.

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{img/modelo_global_nl.png}
	\caption{Modelo global no lineal del accionamiento PMSM implementado en Simulink, compuesto por los subsistemas eléctrico, mecánico y térmico, junto con las transformaciones de Park directa e inversa.}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.95\textwidth]{img/modelo_lti_3x3.png}
...
El modelo LTI equivalente obtenido en los incisos anteriores se basa en imponer la condición de campo orientado:

\begin{equation*}
	i_{ds}^r(t)\equiv 0.
\end{equation*}

Esta condición no se cumple de manera natural en la máquina, sino que debe imponerse mediante una acción de control sobre la tensión virtual del eje directo $v_{ds}^r(t)$. Para determinar dicha ley mínima se parte de la ecuación dinámica del eje $d$:
...
La condición deseada de desacoplamiento es:

\begin{equation*}
	i_{ds}^r(t)\equiv 0,
	\qquad
	\dot{i}_{ds}^r(t)\equiv 0.
\end{equation*}

Reemplazando estas condiciones en \eqref{eq:id_para_ley_minima}, resulta:
...
Por lo tanto, la restricción o ley de control mínima que debe aplicarse sobre el eje directo es:
...
Esta ley cancela el acoplamiento cruzado que aparece en la ecuación del eje $d$, producido por el término $L_q i_{qs}^r(t)P_p\omega_m(t)$. De esta manera, el eje directo no participa en la generación de torque y se logra el desacoplamiento entre el canal de flujo magnético y el canal de torque.

La condición inicial necesaria para que la restricción se cumpla exactamente desde el instante inicial es:
...
Bajo esta hipótesis, si se aplica la ley \eqref{eq:ley_minima_vd}, entonces $i_{ds}^r(t)$ permanece nula para todo $t\geq t_0$. Si la condición inicial no es nula, la corriente de eje directo no se anula instantáneamente, sino que aparece una dinámica residual estable que será analizada en el inciso correspondiente.

\medskip

\noindent
Es importante notar que la ley \eqref{eq:ley_minima_vd} está expresada en coordenadas virtuales rotóricas $qd0$. Sin embargo, el actuador real del sistema es un inversor trifásico que aplica tensiones en los bornes del estator, es decir, tensiones en coordenadas $abc$:
...
Por lo tanto, para implementar físicamente la ley de control mínima es necesario utilizar las transformaciones de Park. En primer lugar, las corrientes medidas en el estator se obtienen en coordenadas reales $abc$ mediante sensores de corriente. A partir de la posición angular del rotor se calcula el ángulo eléctrico:

\begin{equation*}
	\theta_r(t)=P_p\theta_m(t).
\end{equation*}

Con este ángulo se aplica la transformación directa de Park para obtener las corrientes virtuales necesarias para el cálculo de la ley de control:
...
En particular, para la ley mínima se requiere conocer $i_{qs}^r(t)$ y $\omega_m(t)$. La tensión de eje $q$, $v_{qs}^{r*}(t)$, no queda fijada por esta restricción, sino que permanece como la entrada principal del modelo LTI equivalente o como la acción que luego será definida por el controlador de corriente, torque o movimiento.

Además, debido a que el estator se considera trifásico equilibrado y con neutro flotante, la componente homopolar se toma nula:

\begin{equation*}
	v_{0s}^{r*}(t)=0.
\end{equation*}

Entonces, el vector de tensiones virtuales de referencia queda:
...
Para aplicar estas tensiones sobre el inversor trifásico se utiliza la transformación inversa de Park:
...
\begin{equation}
	...
	\begin{bmatrix}
		...
		\cos\left(\theta_r(t)-\frac{2\pi}{3}\right) &
		\sin\left(\theta_r(t)-\frac{2\pi}{3}\right) & 1\\
		\cos\left(\theta_r(t)+\frac{2\pi}{3}\right) &
		\sin\left(\theta_r(t)+\frac{2\pi}{3}\right) & 1
	\end{bmatrix}
	\begin{bmatrix}
		...
		v_{0s}^{r*}(t)
	\end{bmatrix}.
	\label{eq:park_inversa_tensiones}
\end{equation}

Sustituyendo $v_{ds}^{r*}(t)$ por la ley mínima \eqref{eq:ley_minima_vd} y considerando $v_{0s}^{r*}(t)=0$, se obtienen las tensiones de fase que debe aplicar el inversor:

\begin{equation}
	\begin{aligned}
		v_{as}^{*}(t)
		&=
		\cos\theta_r(t)\,v_{qs}^{r*}(t)
		-
		\sin\theta_r(t)\,L_q i_{qs}^{r}(t)P_p\omega_m(t),
		\\[1mm]
		v_{bs}^{*}(t)
		&=
		\cos\left(\theta_r(t)-\frac{2\pi}{3}\right)v_{qs}^{r*}(t)
		-
		\sin\left(\theta_r(t)-\frac{2\pi}{3}\right)
		L_q i_{qs}^{r}(t)P_p\omega_m(t),
		\\[1mm]
		v_{cs}^{*}(t)
		&=
		...
		L_q i_{qs}^{r}(t)P_p\omega_m(t).
	\end{aligned}
	\label{eq:vabc_ley_minima}
\end{equation}

De esta forma, la ley de control mínima se interpreta como una realimentación no lineal parcial de estado. En coordenadas virtuales, la ley fija la tensión $v_{ds}^{r*}(t)$ necesaria para imponer $i_{ds}^r(t)\equiv 0$. En coordenadas físicas, dicha ley se implementa mediante la transformación inversa de Park, generando las tensiones de fase $v_{as}^{*}(t)$, $v_{bs}^{*}(t)$ y $v_{cs}^{*}(t)$ que debe aplicar el inversor trifásico.

En síntesis, el procedimiento de implementación de la ley mínima es:

\begin{enumerate}[leftmargin=*, label=\arabic*.]
	\item Medir las corrientes de fase $i_{as}(t)$, $i_{bs}(t)$ e $i_{cs}(t)$.
	\item Medir la posición angular del rotor $\theta_m(t)$ y calcular $\theta_r(t)=P_p\theta_m(t)$.
	\item Aplicar la transformación directa de Park para obtener $i_{qs}^r(t)$.
	\item Medir o estimar la velocidad $\omega_m(t)$.
	\item Calcular la tensión de desacoplamiento $v_{ds}^{r*}(t)$ mediante \eqref{eq:ley_minima_vd}.
	\item Tomar $v_{0s}^{r*}(t)=0$ y mantener $v_{qs}^{r*}(t)$ como entrada principal de control.
	\item Aplicar la transformación inversa de Park para obtener las tensiones trifásicas $v_{as}^{*}(t)$, $v_{bs}^{*}(t)$ y $v_{cs}^{*}(t)$.
\end{enumerate}

Así, el modelo LTI equivalente trabaja conceptualmente en coordenadas $qd0$, pero la implementación sobre la máquina real o sobre el modelo global no lineal se realiza mediante tensiones trifásicas $abc$.

\paragraph{IV) Implementación de la ley de desacoplamiento en el modelo global no lineal.}

La ley \eqref{eq:ley_minima_vd} se implementa sobre el modelo global no lineal mediante un controlador parcial. Este controlador recibe las variables medidas o estimadas necesarias, calcula la tensión virtual $v_{ds}^{r*}(t)$, forma el vector $\mathbf{v}_{qd0s}^{r*}(t)$ y, mediante la transformación inversa de Park, genera las tensiones trifásicas de referencia para el inversor.

De esta manera, la planta física continúa siendo el modelo no lineal completo en coordenadas $abc$, mientras que el desacoplamiento se realiza en coordenadas virtuales $qd0$. El objetivo de esta implementación es verificar que, al aplicar la ley mínima, la corriente de eje directo se anula o decae rápidamente a cero, permitiendo que la dinámica principal quede dominada por el eje $q$.

\textit{Epígrafe sugerido para figura: Implementación de la ley mínima de desacoplamiento sobre el modelo global no lineal. Se observa la medición de corrientes de fase, la transformación directa de Park, el cálculo de $v_{ds}^{r*}(t)$, la formación del vector de tensiones $qd0$, la transformación inversa de Park y la aplicación de tensiones trifásicas mediante el inversor idealizado.}
...
A partir de (), la dinámica del eje \(q\) desacoplado es
