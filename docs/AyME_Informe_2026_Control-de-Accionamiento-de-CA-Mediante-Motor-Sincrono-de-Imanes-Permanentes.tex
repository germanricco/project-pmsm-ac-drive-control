...
A lo largo de este desarrollo se han deducido tres representaciones matemáticas complementarias para el accionamiento mecatrónico, cada una con un propósito analítico y de diseño específico:

\begin{itemize}
    \item \textbf{Modelo global no lineal (NL):} Representa fielmente la planta física completa en coordenadas virtuales $qd0$. Permite simular el comportamiento transitorio fuertemente acoplado de la dinámica electromecánica de la máquina, los efectos no lineales de la carga y el calentamiento del estator.
    \item \textbf{Modelo lineal con parámetros variables (LPV):} Obtenido mediante linealización Jacobiana, preserva la dependencia de las matrices de estado con el punto de operación (particularmente $I_{ds}^{ro}$). Es la herramienta analítica fundamental para estudiar pequeñas variaciones locales y evaluar la migración de las propiedades dinámicas ante cambios de régimen de operación.
    \item \textbf{Modelo LTI equivalente aumentado:} Resulta de aplicar una estrategia de control vectorial con campo orientado ($I_{ds}^{ro} = 0$) y una linealización por retroalimentación de estados parcial (desacoplamiento no lineal de los ejes $d$ y $q$). Proporciona una planta completamente lineal e independiente del estado de magnetización inicial, la cual constituye el modelo nominal de diseño para la futura síntesis de los lazos de control.
\end{itemize}
