%% PUNTO 4.a - PERFIL POLINOMIAL DE QUINTO ORDEN
% 0--5 s:   q* sube de 0 a 2*pi rad
% 5--10 s:  q* permanece en 2*pi rad
% 10--15 s: q* regresa de 2*pi a 0 rad
%
% Se usa una trayectoria polinomial de quinto orden. La posición,
% velocidad y aceleración son continuas, con velocidad y aceleración
% nulas al inicio y al final de cada desplazamiento.
...
resdir = fullfile(resultadosdir,'punto_4a_quintico');
figdir = resdir;
...
dt_ref = 1e-3;        % resolución temporal de la referencia [s]
...
% Ley normalizada de quinto orden:
% h(s) = 10*s^3 - 15*s^4 + 6*s^5,  0 <= s <= 1.
% Su derivada es h'(s) = 30*s^2 - 60*s^3 + 30*s^4.
% Se entrega w*(t) al modelo para conservar la estructura del caso original.
N_ref = round(t_stop/dt_ref) + 1;
t_w = linspace(0,t_stop,N_ref).';
w_ref = zeros(size(t_w));

mask_ida = t_w >= 0 & t_w <= T_mov;
s_ida = t_w(mask_ida)/T_mov;
w_ref(mask_ida) = (Delta_q/T_mov).* ...
    (30*s_ida.^2 - 60*s_ida.^3 + 30*s_ida.^4);

mask_vuelta = t_w >= t_retorno & t_w <= t_stop;
s_vuelta = (t_w(mask_vuelta)-t_retorno)/T_mov;
w_ref(mask_vuelta) = -(Delta_q/T_mov).* ...
    (30*s_vuelta.^2 - 60*s_vuelta.^3 + 30*s_vuelta.^4);

% Corrección numérica mínima para que la integración de cada movimiento
% produzca exactamente +/-Delta_q con la malla temporal elegida.
area_ida = trapz(t_w(mask_ida),w_ref(mask_ida));
area_vuelta = trapz(t_w(mask_vuelta),w_ref(mask_vuelta));
w_ref(mask_ida) = w_ref(mask_ida)*(Delta_q/area_ida);
w_ref(mask_vuelta) = w_ref(mask_vuelta)*(-Delta_q/area_vuelta);

ts_w = timeseries(w_ref,t_w,'Name','w_t');
ts_w = setinterpmethod(ts_w,'linear');

% Valores teóricos máximos de la trayectoria quintica.
s_eval = linspace(0,1,10001).';
h1 = 30*s_eval.^2 - 60*s_eval.^3 + 30*s_eval.^4;
h2 = 60*s_eval - 180*s_eval.^2 + 120*s_eval.^3;
w_max_teorica = (Delta_q/T_mov)*max(abs(h1));
alpha_max_teorica = (Delta_q/T_mov^2)*max(abs(h2));

fprintf('Perfil polinomial de quinto orden:\n');
fprintf('  w_max teórica     = %.6f rad/s\n',w_max_teorica);
fprintf('  alpha_max teórica = %.6f rad/s^2\n',alpha_max_teorica);
...
disp('Ejecutando punto 4.a con perfil polinomial de quinto orden...');
out = sim(in);
...
% Los intervalos se detectan automáticamente a partir de alpha_l_ref.
% Esto evita considerar como aceleración los instantes próximos al máximo
% de velocidad, donde la aceleración quintica pasa por cero.
opciones_metricas.umbral_aceleracion_rel = 0.05;
...
[fig,ax] = plotter.plotTime(datos.t,[datos.q_ref datos.q datos.q_est], ...
    Title="Punto 4.a - Perfil quíntico: posición", ...
...
[fig,ax] = plotter.plotTime(datos.t,[datos.wl_ref datos.wl datos.wl_est], ...
    Title="Punto 4.a - Perfil quíntico: velocidad", ...
...
resultado_4a = struct;
resultado_4a.descripcion = 'Punto 4.a - perfil polinomial de quinto orden';
resultado_4a.parametros_perfil = struct('Delta_q',Delta_q,'T_mov',T_mov, ...
    'dt_ref',dt_ref,'w_max_teorica',w_max_teorica, ...
    'alpha_max_teorica',alpha_max_teorica);
...
save(fullfile(resdir,'resultado_punto_4a_quintico.mat'), ...
    'resultado_4a','-v7.3');
