%% PUNTO 4.a - SEGUIMIENTO DE LA CONSIGNA ORIGINAL
% 0--5 s:   q* sube de 0 a 2*pi rad
% 5--10 s:  q* permanece en 2*pi rad
% 10--15 s: q* regresa de 2*pi a 0 rad
%
% Se usa una velocidad casi rectangular, con rampas numéricas de 1 ms.
...
resdir = fullfile(resultadosdir,'punto_4a_original');
figdir = resdir;
...
T_mov = 5.0;
t_stop = 15.0;
w_mov = 2*pi/(T_mov-t_rampa);

% q_t queda nula: set_point integra w_t.
ts_q = timeseries([0;0],[0;t_stop],'Name','q_t');
ts_q = setinterpmethod(ts_q,'zoh');

t_w = [0; t_rampa; T_mov-t_rampa; T_mov; 10; ...
       10+t_rampa; 15-t_rampa; 15];
w_ref = [0; w_mov; w_mov; 0; 0; -w_mov; -w_mov; 0];
ts_w = timeseries(w_ref,t_w,'Name','w_t');
ts_w = setinterpmethod(ts_w,'linear');

% Intervalos en los que la referencia cambia de velocidad. Se pasan
% explícitamente al analizador para evaluar la especificación de corriente
% máxima "2 A RMS (corta duración, aceleración)".
...
disp('Ejecutando punto 4.a con señales estimadas...');
out = sim(in);
...
[fig,ax] = plotter.plotTime(datos.t,[datos.q_ref datos.q datos.q_est], ...
    Title="Punto 4.a - Seguimiento de posición", ...
...
[fig,ax] = plotter.plotTime(datos.t,[datos.wl_ref datos.wl datos.wl_est], ...
    Title="Punto 4.a - Seguimiento de velocidad", ...
...
save(fullfile(resdir,'resultado_punto_4a_original.mat'), ...
    'resultado_4a','-v7.3');
